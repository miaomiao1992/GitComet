...
pub(crate) enum ConflictPerfSpan {
    RenderThreeWayRows,
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) enum ConflictPerfRenderLane {
    ThreeWay,
    ResolverDiff,
    ResolvedPreview,
}
...
#[derive(Clone, Copy, Debug, Default, Eq, PartialEq)]
#[allow(dead_code)]
pub(crate) struct ConflictPerfSnapshot {
...
#[inline]
pub(crate) fn span(span: ConflictPerfSpan) -> PerfScope {
...
#[inline]
#[allow(dead_code)]
pub(crate) fn snapshot() -> ConflictPerfSnapshot {
    #[cfg(debug_assertions)]
    {
        ConflictPerfSnapshot {
...
#[inline]
#[cfg(debug_assertions)]
fn span_stats(span: ConflictPerfSpan) -> &'static AtomicSpanStats {
    match span {
        ConflictPerfSpan::RenderThreeWayRows => &RENDER_THREE_WAY_ROWS_SPAN,
        ConflictPerfSpan::RenderResolverDiffRows => &RENDER_RESOLVER_DIFF_ROWS_SPAN,
        ConflictPerfSpan::RenderResolvedPreviewRows => &RENDER_RESOLVED_PREVIEW_ROWS_SPAN,
        ConflictPerfSpan::RecomputeResolvedOutline => &RECOMPUTE_RESOLVED_OUTLINE_SPAN,
        ConflictPerfSpan::StyledTextBuild => &STYLED_TEXT_BUILD_SPAN,
        ConflictPerfSpan::SyntaxHighlighting => &SYNTAX_HIGHLIGHTING_SPAN,
        ConflictPerfSpan::WordQueryHighlighting => &WORD_QUERY_HIGHLIGHTING_SPAN,
    }
}

#[inline]
#[cfg(debug_assertions)]
fn row_stats(lane: ConflictPerfRenderLane) -> &'static AtomicRowBatchStats {
    match lane {
        ConflictPerfRenderLane::ThreeWay => &RENDER_THREE_WAY_ROWS_BATCH,
        ConflictPerfRenderLane::ResolverDiff => &RENDER_RESOLVER_DIFF_ROWS_BATCH,
        ConflictPerfRenderLane::ResolvedPreview => &RENDER_RESOLVED_PREVIEW_ROWS_BATCH,
    }
}
...
mod tests {
    ...
    fn snapshot_exposes_recorded_row_batches() {
        reset();
        record_row_batch(ConflictPerfRenderLane::ThreeWay, 12, 9);
        ...
        #[cfg(not(debug_assertions))]
        {
            assert_eq!(snapshot, ConflictPerfSnapshot::default());
        }
    }
}
